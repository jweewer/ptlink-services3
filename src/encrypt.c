...
{
  unsigned int i, indexp, partLen;

  /* Compute number of bytes mod 64 */
  indexp = (unsigned int)((context->count[0] >> 3) & 0x3F);
...
*/
  if (inputLen >= partLen) {
 memcpy
   ((POINTER)&context->buffer[indexp], (POINTER)input, partLen);
 ...
 i = 0;
  ...
  memcpy
 ((POINTER)&context->buffer[indexp], (POINTER)&input[i],
  inputLen-i);
}
...
*/
  indexp = (unsigned int)((context->count[0] >> 3) & 0x3f);
  padLen = (indexp < 56) ? (56 - indexp) : (120 - indexp);
...
void* encrypted_password(char *passwd)
{
  static char buf[32];
  encrypt_md5(passwd, strlen(passwd), buf);
  return buf;
}
